package com.youengineering.openapi.document.reader.jaxrs;
...
public class JaxrsReader extends AbstractReader {
    ...
    @Override
    public void enrich(OpenAPI openAPI, Set<Class<?>> apiClasses) {
        for (Class<?> apiClass : apiClasses) {
            enrich(openAPI, apiClass);
        }
    }

    private void enrich(OpenAPI openAPI, Class<?> apiClass) {
        enrich(openAPI, apiClass, "", null, false, new LinkedList<>());
    }
    ...
    private void enrich(
        ...
        for (Method method : filteredMethods) {
            ...
            if (operationPath != null) {
                ...
                if (httpMethod != null) {
                    Operation operation = parseAndAddMethod(openAPI, apiClass, method, httpMethod, operationPath, parentParameters);
                    if (isSubResource(httpMethod, method)) {
                        Class<?> responseClass = method.getReturnType();
                        log.debug("handling sub-resource method " + method.toString() + " -> " + responseClass);
                        enrich(openAPI, responseClass, operationPath, httpMethod, true, operation.getParameters());
                    }
                }
            }
        }
