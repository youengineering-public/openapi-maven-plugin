...
public abstract class AbstractReader {
    ...
    private void initializeSwaggerExtensions(OpenAPI swagger) {
        List<OpenAPIExtension> swaggerExtensions = getSwaggerExtensions();
        for (OpenAPIExtension swaggerExtension : swaggerExtensions) {
    ...
    public abstract void enrich(OpenAPI swagger, Set<Class<?>> classes);
    ...
    protected Operation parseAndAddMethod(OpenAPI swagger, Class<?> clazz, Method method, String httpMethodName, String fullPath, Collection<Parameter> parentParameters) {
        ...
        if (operationAnnotation != null) {
            ...
            for (ApiResponse response : responses.values()) {
                for (MediaType mediaType : response.getContent().values()) {
                    swagger.getComponents().addSchemas(mediaType.getSchema().getName(), mediaType.getSchema());
                }
            }
        }
        ...
        if (!responseType.equals(Void.class) && !responseType.equals(void.class) && hasResponseContent(responseType, method, httpMethodName)) {
            ...
            for (Map.Entry<String, Schema> referencedSchemaEntry : resolvedSchema.referencedSchemas.entrySet()) {
                swagger.getComponents().addSchemas(referencedSchemaEntry.getKey(), referencedSchemaEntry.getValue());
            }
        }
        ...
        for (int i = 0; i < parameterTypes.length; i++) {
            ...
            ResolvedParameter resolvedParameter = getParameters(type, annotations, swagger.getComponents());
        ...
        PathItem path = swagger.getPaths().computeIfAbsent(operationPath, op -> new PathItem());
